...
class Es7SearchService(
    search_service.SearchService, search_service_type="es7_search_service"
):
    ...
    def get_analyzed_fields_from_mapping(
        ...
        for prop_name, prop_values in properties.items():
            if "properties" in prop_values:
                res = Es7SearchService.get_analyzed_fields_from_mapping(
    ...
    def _init_field_mapping(
        ...
        for (alias, index) in aliases:
            if (
                ...
                field_mapping[
                    alias
                ] = Es7SearchService.get_analyzed_fields_from_mapping(
                    mapping[index]["mappings"]["entry"]["properties"]
                )
                sortable_fields[
                    alias
                ] = Es7SearchService.create_sortable_map_from_mapping(
                    mapping[index]["mappings"]["entry"]["properties"]
                )
