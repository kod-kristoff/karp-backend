...
from karp.domain.errors import IncompleteQuery, UnsupportedQuery, UnsupportedField
...
class EsSearchIndex(SearchIndex):
    ...
    def publish_index(self, alias_name, index_name):
        ...
        self.on_publish_resource(alias_name=alias_name, index_name=index_name)
    ...
    #     self.update_fields_on_publish = EsSearchIndex.UpdateFieldsOnPublish(self)
    ...
    def get_analyzed_fields_from_mapping(
        ...
        for prop_name, prop_values in properties.items():
            if "properties" in prop_values:
                res = EsSearchIndex.get_analyzed_fields_from_mapping(
    ...
    def _init_field_mapping(
        ...
        for (alias, index) in aliases:
            if (
                ...
                field_mapping[alias] = EsSearchIndex.get_analyzed_fields_from_mapping(
                    mapping[index]["mappings"]["entry"]["properties"]
                )
                sortable_fields[alias] = EsSearchIndex.create_sortable_map_from_mapping(
                    mapping[index]["mappings"]["entry"]["properties"]
                )
    ...
    def on_publish_resource(self, alias_name: str, index_name: str):
        ...
        if (
            ...
            self.analyzed_fields[
                alias_name
            ] = EsSearchIndex.get_analyzed_fields_from_mapping(
                mapping[index_name]["mappings"]["entry"]["properties"]
            )
            self.sortable_fields[
                alias_name
            ] = EsSearchIndex.create_sortable_map_from_mapping(
                mapping[index_name]["mappings"]["entry"]["properties"]
            )
