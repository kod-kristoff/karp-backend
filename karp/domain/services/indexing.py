...
def pre_process_resource(
    indexer: SearchIndex, resource_obj: Resource,
    ...
    return [
        (
            ...
            transform_to_index_entry(
                indexer, resource_obj, json.loads(entry.body), fields
            ),
        )
...
def reindex(
    ...
    try:
        index_name = indexer.create_index(resource_id, resource_obj.config)
    except NotImplementedError:
        _logger.error("No Index module is loaded. Check your configurations...")
        sys.exit(errors.NoIndexModuleConfigured)
    if not search_entries:
        search_entries = pre_process_resource(indexer, resource_obj)
    add_entries(indexer, index_name, search_entries, update_refs=False)
    indexer.publish_index(resource_id, index_name)


def publish_index(
    indexer: SearchIndex, resource_id: str, version: Optional[int] = None
) -> None:
    reindex(indexer, resource_id, version=version)
    if version:
...
def add_entries(
    ...
    indexer.add_entries(resource_id, entries)
    if update_refs:
        _update_references(
            indexer, resource_id, [entry_id for (entry_id, _, _) in entries]
        )


def delete_entry(indexer: SearchIndex, resource_id: str, entry_id: str) -> None:
    indexer.delete_entry(resource_id, entry_id)
    _update_references(indexer, resource_id, [entry_id])


def _update_references(
    indexer: SearchIndex, resource_id: str, entry_ids: List[str]
) -> None:
...
def transform_to_index_entry(
    ...
    _transform_to_index_entry(indexer, resource, src_entry, index_entry, fields)
    return index_entry
    ...
    for field_name, field_conf in fields:
        if field_conf.get("virtual", False):
            res = _evaluate_function(
                indexer, field_conf["function"], _src_entry, resource
            )
            if res:
