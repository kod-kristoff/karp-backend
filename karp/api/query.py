...
def get_entries_by_id(resource_id: str, entry_ids: str):
    response = context.search_index.search_ids(request.args, resource_id, entry_ids)
...
def query(resources: str):
    ...
    try:
        q = context.search_index.build_query(request.args, resources)
        print("query::q={q}".format(q=q))
        response = context.search_index.search_with_query(q)
...
def query_split(resources: str):
    ...
    resourcemgr.check_resource_published(resource_list)
    try:
        query = context.search_index.build_query(request.args, resources)
        query.split_results = True
        print("query={}".format(query))
        response = context.search_index.search_with_query(query)
    except errors.KarpError as e:
        _logger.exception(
