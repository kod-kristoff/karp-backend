...
from karp.infrastructure.elasticsearch7.es7_search_service import (
    Es7SearchService,
    UnsupportedField,
    _create_es_mapping,
)
...
@pytest.fixture(name="es7_search")
def fixture_es7_search():
    es_mock = mock.Mock()
    es_mock.cat.aliases.return_value = ".one\n"
    return Es7SearchService(es_mock)
...
def test_create_empty(es_mock):
    es_search = Es7SearchService(es_mock)
...
def test_parse_mapping_empty(
    ...
    sortable_map = Es7SearchService.create_sortable_map_from_mapping(properties)
...
def test_create_empty_object_returns_index_entry(es7_search):

    index_entry = es7_search.create_empty_object()
...
def test_assign_field_adds_field_to_index_entry(es7_search):

    index_entry = es7_search.create_empty_object()

    es7_search.assign_field(index_entry, "field", {})
    assert index_entry.entry["field"] == {}


def test_create_empty_list_returns_list(es7_search):

    index_list = es7_search.create_empty_list()

    assert isinstance(index_list, list)
    assert index_list == []


def test_add_to_list_field_adds_to_list(es7_search):

    index_list = es7_search.create_empty_list()

    es7_search.add_to_list_field(index_list, "a")

    assert index_list == ["a"]

    es7_search.add_to_list_field(index_list, {"b": "c"})
