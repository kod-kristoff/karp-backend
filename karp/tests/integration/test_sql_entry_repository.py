...
def test_save_entry_with_slash_to_entry_repo(entry_repo):
    entry = factories.EntryFactory(entry_id="high/low")
    entry_repo.save(entry)

    assert entry_repo.by_id(entry.id).entry_id == entry.entry_id
    assert entry_repo.by_entry_id(entry.entry_id).id == entry.id
    assert entry_repo.by_entry_id(entry.entry_id).entry_id == entry.entry_id
...
def test_discard_entry_and_insert_new(entry_repo):
    ...
    assert (
        entry_repo.by_entry_id(entry2.entry_id, version=1).entity_id == entry2.entity_id
    )
